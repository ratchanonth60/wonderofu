...
/// Builds a non-streaming tool-use request for Vertex AI Gemini models.
pub(super) fn build_vertex_tool_use_request(
    resolved: &ResolvedProviderExecution,
    ...
    let body = gemini::build_gemini_tool_use_body(request);
...
/// Parses a Vertex AI Gemini function-calling response.
pub(super) fn parse_vertex_tool_use_response(
    resolved: &ResolvedProviderExecution,
    body: &str,
) -> Result<ToolUseResponse> {
    gemini::parse_gemini_tool_use_response(resolved, body)
}
