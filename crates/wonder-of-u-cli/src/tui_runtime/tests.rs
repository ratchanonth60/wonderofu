...
fn task_completion_injects_xml_notification_into_transcript() {
    ...
    // Exactly one TaskNotification message should have been appended.
    ...
    // The payload must contain the required XML tags.
    let xml = controller
        .state
        .messages
        .iter()
...
fn tui_notification_overlay_preserved_alongside_xml_injection() {
    ...
    // The human-facing TUI notification must still be present.
    let notifications = controller.view().notifications;
    assert_eq!(
        notifications.len(),
        1,
    ...
    // AND the XML injection must also be present (tested above).
    assert!(
        controller
            .state
            .messages
...
fn repeated_polling_does_not_duplicate_xml_notification() {
    ...
    // Static registry leak – fine in test binaries.
    let registry = commands::registry(Some(dir.clone())).expect("registry");
    let registry: &'static _ = Box::leak(Box::new(registry));
    ...
    // Poll again with the same completed state multiple times.
    controller
        .refresh_runtime_state()
...
fn injected_task_id_tracked_in_state_set() {
    ...
    // The task id must be recorded in the idempotence set.
    let msg = controller
        .state
        .messages
        .iter()
