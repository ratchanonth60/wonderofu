...
/// Creates a controller with no provider config so `provider_readiness()` returns
/// `Unconfigured`.
...
/// Creates a controller with a valid provider config so `provider_readiness()`
/// returns `Ready`.
...
fn controller_cancel_setup_sets_session_flag_and_prevents_reopen() {
    ...
    // Overlay was auto-opened; now the user presses Esc to cancel.
    controller.cancel_setup_overlay().expect("cancel_setup_overlay");
    ...
    // Calling maybe_auto_open_setup again must not re-open the overlay.
    controller
