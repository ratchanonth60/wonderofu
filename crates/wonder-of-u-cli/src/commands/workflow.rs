...
use wonder_of_u_core::{
    AgentDefinition, AgentDefinitionSource, Command, CommandContext, CommandInvocation,
    CommandKind, CommandOutput, CommandSpec, FeatureFlag, PermissionDecision, PermissionMode,
    PermissionRequest, PermissionRuleSource, RawDefinitionFields, Result, TaskId, TaskKind,
    TaskState, TaskStatus, ToolPermissionContext, ToolSpec, WonderError,
...
struct DefinitionsArgs {
    #[command(subcommand)]
    command: DefinitionsSubcommand,
}

#[derive(Debug, Subcommand)]
enum DefinitionsSubcommand {
...
impl Command for AgentsCommand {
    ...
    async fn execute(
        ...
        match args.command.unwrap_or(AgentsSubcommand::Status) {
            ...
            AgentsSubcommand::Definitions(args) => self.definitions(context, args),
        }
    }
}
...
impl AgentsCommand {
    ...
    // ── definitions sub-surface ───────────────────────────────────────────────

    fn definitions(&self, context: CommandContext, args: DefinitionsArgs) -> Result<CommandOutput> {
        let root = resolve_project_root(&context.cwd);
        match args.command {
            DefinitionsSubcommand::List => self.definitions_list(&root),
            DefinitionsSubcommand::Show(a) => self.definitions_show(&root, &a.id),
            DefinitionsSubcommand::Create(a) => self.definitions_create(&root, a),
            DefinitionsSubcommand::Edit(a) => self.definitions_edit(&root, a),
        }
    }

    fn definitions_list(&self, root: &Path) -> Result<CommandOutput> {
    ...
    fn definitions_show(&self, root: &Path, id: &str) -> Result<CommandOutput> {
        let loader = AgentDefinitionLoader::new(root);
        let (catalog, warnings) = loader.build_catalog()?;
        let def = catalog
            .resolve_alias(id)
    ...
    fn definitions_edit(&self, root: &Path, args: DefinitionsEditArgs) -> Result<CommandOutput> {
...
mod tests {
    ...
    /// Builds a `CommandContext` whose `cwd` points at the test project root.
    fn agents_test_context(root: &Path) -> CommandContext {
        test_context(root)
    }

    fn agents_cmd() -> super::AgentsCommand {
        super::AgentsCommand::new(None)
    }

    // Helper: invoke agents definitions list synchronously.
    fn def_list(root: &Path) -> String {
        let cmd = agents_cmd();
        let ctx = agents_test_context(root);
        let output = cmd.definitions_list(root).expect("definitions list");
        let _ = ctx;
        match output {
    ...
    fn def_show(root: &Path, id: &str) -> String {
        let cmd = agents_cmd();
        match cmd.definitions_show(root, id).expect("definitions show") {
            CommandOutput::Text(t) => t,
            other => panic!("expected Text output, got {other:?}"),
        }
    }

    fn def_create(root: &Path, args: super::DefinitionsCreateArgs) -> String {
        let cmd = agents_cmd();
        match cmd
            .definitions_create(root, args)
            .expect("definitions create")
        {
            CommandOutput::Text(t) => t,
            other => panic!("expected Text output, got {other:?}"),
        }
    ...
    fn def_create_err(root: &Path, args: super::DefinitionsCreateArgs) -> String {
        let cmd = agents_cmd();
        cmd.definitions_create(root, args)
            .expect_err("expected create to fail")
            .to_string()
    }

    fn def_edit(root: &Path, args: super::DefinitionsEditArgs) -> String {
        let cmd = agents_cmd();
        match cmd.definitions_edit(root, args).expect("definitions edit") {
            CommandOutput::Text(t) => t,
            other => panic!("expected Text output, got {other:?}"),
        }
    }

    fn def_edit_err(root: &Path, args: super::DefinitionsEditArgs) -> String {
        let cmd = agents_cmd();
        cmd.definitions_edit(root, args)
            .expect_err("expected edit to fail")
            .to_string()
    }

    fn create_args_minimal(name: &str, desc: &str, prompt: &str) -> super::DefinitionsCreateArgs {
        super::DefinitionsCreateArgs {
            ...
            max_turns: None,
            target: super::DefinitionTarget::DotClaude,
            overwrite: false,
        }
    }
    ...
    fn agents_definition_show_unknown_returns_error() {
        ...
        let cmd = agents_cmd();
        let err = cmd
            .definitions_show(&root, "completely-unknown-xyz")
            .unwrap_err();
        assert!(
    ...
    fn agents_definition_create_in_agents_dir() {
        ...
        args.target = super::DefinitionTarget::Agents;
        let out = def_create(&root, args);
        assert!(out.contains("created=true"), "got:\n{out}");
    ...
    fn agents_status_runtime_behavior_unaffected() {
        ...
        // after adding the Definitions subcommand.
        let cmd = agents_cmd();
...
fn resolve_project_root(cwd: &Path) -> PathBuf {
    if let Some(capture) = capture_git(cwd, &["rev-parse", "--show-toplevel"]) {
        if capture.success && !capture.stdout.is_empty() {
            return PathBuf::from(&capture.stdout);
        }
    }
    cwd.to_path_buf()
}
